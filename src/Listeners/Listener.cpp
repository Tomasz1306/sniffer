...
void Listener::openListener(){
    ...
    if (this->dev->isOpened()) {
        LogController::getInstance()->addLog(Utils::getTime(), "ERROR DEVICE IS ACTUALLY OPEN", LogType::ERROR);
        return;
    }
    if (this->dev->open()) {
        LogController::getInstance()->addLog(Utils::getTime(), "DEVICE IS OPENED AT INTERFACE", LogType::SUCCESFULL);
    } else {
        LogController::getInstance()->addLog(Utils::getTime(), "ERROR CANT OPEN DEVICE", LogType::ERROR);
    }
}
...
void Listener::closeListener(){
    ...
    LogController::getInstance()->addLog(Utils::getTime(), "DEVICE IS CLOSED", LogType::SUCCESFULL);
    this->dev->close();
}
...
void Listener::startCapturePackets(){
    ...
    LogController::getInstance()->addLog(Utils::getTime(), "START CAPTURING PACKETS", LogType::SUCCESFULL);
...
void Listener::stopCapturePackets(){
    ...
    if (!this->dev->captureActive()) {
        LogController::getInstance()->addLog(Utils::getTime(), "ERROR DEVICE IS NOT CAPTURING", LogType::ERROR);
        //WRITE TO LOG 
        return;
    }
    LogController::getInstance()->addLog(Utils::getTime(), "STOP CAPTURING PACKETS", LogType::SUCCESFULL);
...
void Listener::setDeviceByName(std::string name) {
    if (this->dev != nullptr) {
        if (this->dev->isOpened()) {
            LogController::getInstance()->addLog(Utils::getTime(), "DEVICE IS OPEN ", LogType::SUCCESFULL);
            this->dev->close();
            this->dev = nullptr;
        }

        if (this->dev->captureActive()) {
            LogController::getInstance()->addLog(Utils::getTime(), "DEVICE IS CAPTURING", LogType::SUCCESFULL);
            return;
        }
    }
    ...
    if (this->dev != nullptr) {
        LogController::getInstance()->addLog(Utils::getTime(), "DEVICE IS SELECTED", LogType::SUCCESFULL);
    }
}
