...
AnalyzerController::AnalyzerController(std::shared_ptr<AnalyzerModel> model,
...
void AnalyzerController::analyzePackets(std::vector<CapturedPackets> packets) {
  this->analyzerThread = std::make_shared<std::thread>(
      &AnalyzerController::analyzeCapturedPackets, this, packets);
  this->analyzerThread->detach();
}
void AnalyzerController::analyzeCapturedPackets(
...
void AnalyzerController::display() { this->view->draw(shared_from_this()); }

void AnalyzerController::setMainController(
...
void AnalyzerController::analyzePacketForPortScan(CapturedPackets packet) {
  ...
  if (ipLayer != nullptr && tcpLayer != nullptr) {
    ...
    this->sourceIpToPorts[sourceIp].insert(destPort);
    if (this->sourceIpToPorts[sourceIp].size() > 10) {
      std::ostringstream oss;
      oss << "Potential port scan detected from IP: " << sourceIp << " "
          << "Date: " << packet.captureTime;
...
void AnalyzerController::analyzePacketForPingSweep(CapturedPackets packet) {
  ...
  if (icmpLayer != nullptr &&
      icmpLayer->getMessageType() == pcpp::ICMP_ECHO_REQUEST) {
    std::string sourceIp = packet.packet.getLayerOfType<pcpp::IPv4Layer>()
                               ->getSrcIPAddress()
                               .toString();
    pingRequests[sourceIp]++;
    if (pingRequests[sourceIp] > 10 &&
        warnings.find(sourceIp) == warnings.end()) {
      std::ostringstream oss;
...
void AnalyzerController::analyzePacketForARPSpoofing(CapturedPackets packet) {
  ...
  if (arpLayer != nullptr) {
    ...
    if (arpMapping[srcIP] != srcMac) {
      std::ostringstream oss;
      oss << "Potential ARP spoofing detected: IP " << srcIP
          << " now maps to MAC " << srcMac << " (was " << arpMapping[srcIP]
          << ")";
...
void AnalyzerController::analyzePacketForBruteForce(
    const CapturedPackets packet) {
  auto ipLayer = packet.packet.getLayerOfType<pcpp::IPv4Layer>();
  auto tcpLayer = packet.packet.getLayerOfType<pcpp::TcpLayer>();
  if (ipLayer != nullptr && tcpLayer != nullptr) {
    std::string sourceIp = ipLayer->getSrcIPAddress().toString();
    ...
    if (destPort == 22) { // Port 22 dla SSH
      ...
      if (this->bruteForceAttempts[key] > 20 &&
          this->warnings.find(key) == this->warnings.end()) {
        std::ostringstream oss;
        oss << "Potential SSH brute force attempt from IP: " << sourceIp
            << " on " << packet.captureTime;
        this->warnings[packet.captureTime] = Warning(sourceIp, oss.str());
      }
    }
  }
...
void AnalyzerController::analyzePacketHttp(CapturedPackets packet) {
  ...
  if (httpLayer != nullptr) {
    ...
    pcpp::PayloadLayer *payloadLayer =
        packet.packet.getLayerOfType<pcpp::PayloadLayer>();
    if (payloadLayer != nullptr) {
      const uint8_t *payload =
          packet.packet.getLayerOfType<pcpp::PayloadLayer>()->getPayload();
      int payloadSize =
          packet.packet.getLayerOfType<pcpp::PayloadLayer>()->getPayloadLen();
      if (payload) {
    ...
    if (isXSS && isSQL) {
      warnings[packet.captureTime] = Warning(
          sourceIp, std::string("Potential SQL Injection and XSS attempt: " +
                                asciiString + " at " + packet.captureTime));
    } else if (isXSS) {
      warnings[packet.captureTime] = Warning(
          sourceIp, std::string("Potential XSS attempt: " + asciiString +
                                " at " + packet.captureTime));
    } else {
      warnings[packet.captureTime] = Warning(
          sourceIp, std::string("Potential SQL Injection attempt: " +
                                asciiString + " at " + packet.captureTime));
    }
  } else {
  }
