...
DataBaseController::DataBaseController(std::shared_ptr<DataBaseModel> model,
...
void DataBaseController::display() { this->view->draw(shared_from_this()); }

void DataBaseController::connectToDataBase(std::string host, std::string port,
                                           std::string username,
                                           std::string password) {
  try {
    this->connection =
        this->driver->connect(host + ":" + port, username, password);
    for (int i = 0; i < 2; i++) {
      try {
        int session_id{0};
        auto connection =
        ...
        LogController::getInstance()->addLog(Utils::getTime(), e.what(),
                                             LogType::ERROR);
      } catch (const std::exception &e) {
        std::cerr << "Exception in initializeDbThreadPool: " << e.what()
                  << std::endl;
        LogController::getInstance()->addLog(Utils::getTime(), e.what(),
                                             LogType::ERROR);
      }
    ...
    LogController::getInstance()->addLog(Utils::getTime(),
                                         ...
                                         LogType::SUCCESFULL);
    this->model->setConnection(host, port, username, password,
                               Utils::getTime());
...
void DataBaseController::loadDatabases() {
  ...
  try {
    ...
    LogController::getInstance()->addLog(Utils::getTime(), "LOADED DATABASES",
                                         LogType::SUCCESFULL);
  } catch (sql::SQLException &e) {
    LogController::getInstance()->addLog(Utils::getTime(), e.what(),
                                         LogType::WARNING);
...
void DataBaseController::newDatabase(std::string database) {
  try {
    std::string query = std::string("CREATE DATABASE IF NOT EXISTS ") +
                        std::string("sniffer_") + database;
    this->stmt = this->connection->createStatement();
    this->stmt->execute(query);
    LogController::getInstance()->addLog(
        Utils::getTime(), "CREATED NEW DATABASE", LogType::SUCCESFULL);
  } catch (sql::SQLException &e) {
    LogController::getInstance()->addLog(Utils::getTime(), e.what(),
                                         LogType::WARNING);
  }
...
void DataBaseController::initializeDbThreadPool() {
  ...
  LogController::getInstance()->addLog(
      Utils::getTime(), "Before Database thread started", LogType::SUCCESFULL);
    ...
    while (true) {
      CapturedPackets packet;
      {
        ...
        database_cv.wait(lock, [this]() {
          return !this->mainController->getCapturedPacketVectorDatabase()
                      .empty() ||
                 this->done || !this->areConnectionsForThreadInitialized;
        });
        ...
        if (!this->mainController->getCapturedPacketVectorDatabase().empty()) {
          ...
          packet =
              this->mainController->getCapturedPacketVectorDatabase().back();
  ...
  try {
    for (int i = 0; i < numThreads; ++i) {
      ...
      std::thread t([this, workerFunction]() { workerFunction(); });
      {
        ...
        stmt->execute(
            "USE " +
            this->model->getDatabases()[this->model->getSelectedDatabaseIndex()]
                .first);
        LogController::getInstance()->addLog(
            Utils::getTime(),
            "THREAD: " + std::to_string(i) + "USING DATABASE: " +
                this->model
                    ->getDatabases()[this->model->getSelectedDatabaseIndex()]
                    .first,
            LogType::SUCCESFULL);
  ...
  LogController::getInstance()->addLog(
      Utils::getTime(), "Database thread started", LogType::SUCCESFULL);
...
void DataBaseController::createTables() {
  auto createTableTask = [this]() {
    try {
      ...
      int session_id{0};
      this->prep_stmt = this->connection->prepareStatement(
          "INSERT INTO Sessions(session_date) VALUES(STR_TO_DATE(?, '%d-%m-%Y "
          "%H:%i:%s'))");
      ...
      for (auto &device :
           this->mainController->getDeviceController()->getDevices()) {
        std::string query =
            R"(INSERT INTO Interfaces(interface_name, interface_ipv4,
        ...
        this->prep_stmt->setString(6,
                                   device->getDnsServers().begin()->toString());
        ...
        this->currentSessionInterfacesIds.emplace(interface_id,
                                                  device->getName());
      }

      LogController::getInstance()->addLog(
          Utils::getTime(), "CREATED ALL TABLES", LogType::SUCCESFULL);
    } catch (sql::SQLException &e) {
      LogController::getInstance()->addLog(Utils::getTime(), e.what(),
                                           LogType::ERROR);
    }
...
void DataBaseController::setMainController(
...
std::string DataBaseController::getHost() { return this->model->getHost(); }

std::string DataBaseController::getPort() { return this->model->getPort(); }
...
void DataBaseController::insertNewPacket(CapturedPackets &packet,
  ...
  try {
    ...
    if (packet.packet.isPacketOfType(pcpp::Ethernet)) {
      buildEthernetQuery(packet, ethLayer, firstField, ethernet_id,
                         packetsQuery, values, connection_local, prep_stmt,
                         stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::IPv4)) {
      buildIpv4Query(packet, ipv4Layer, firstField, ipv4_id, packetsQuery,
                     values, connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::IPv6)) {
      buildIpv6Query(packet, ipv6Layer, firstField, ipv6_id, packetsQuery,
                     values, connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::ARP)) {
      buildArpQuery(packet, arpLayer, firstField, arp_id, packetsQuery, values,
                    connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    ...
    if (packet.packet.isPacketOfType(pcpp::TCP)) {
      buildTcpQuery(packet, tcpLayer, firstField, tcp_id, packetsQuery, values,
                    connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::UDP)) {
      buildUdpQuery(packet, udpLayer, firstField, udp_id, packetsQuery, values,
                    connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::HTTPRequest)) {
      buildHttpRequestQuery(packet, httpRequestLayer, firstField,
                            httpRequest_id, packetsQuery, values,
                            connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::HTTPResponse)) {
      buildHttpResponseQuery(packet, httpResponseLayer, firstField,
                             httpResponse_id, packetsQuery, values,
                             connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    auto ftpLayer = packet.packet.getLayerOfType<pcpp::FtpLayer>();
    if (auto isftpRequestLayer =
            dynamic_cast<pcpp::FtpRequestLayer *>(ftpLayer)) {
      buildFtpRequestQuery(packet, ftpRequestLayer, firstField, ftpRequest_id,
                           packetsQuery, values, connection_local, prep_stmt,
                           stmt, res);
      isProtocolSupported = true;
    } else if (auto isftpResponseLayer =
                   dynamic_cast<pcpp::FtpResponseLayer *>(ftpLayer)) {
      buildFtpRequestQuery(packet, ftpRequestLayer, firstField, ftpRequest_id,
                           packetsQuery, values, connection_local, prep_stmt,
                           stmt, res);
      isProtocolSupported = true;
    ...
    if (packet.packet.isPacketOfType(pcpp::SSH)) {
      // this->buildSshQuery(packet, sshLayer, firstField, ssh_id, packetsQuery,
      // values);
    }
    if (packet.packet.isPacketOfType(pcpp::Telnet)) {
      buildTelnetQuery(packet, telnetLayer, firstField, telnet_id, packetsQuery,
                       values, connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::DNS)) {
      buildDnsQuery(packet, dnsLayer, firstField, dns_id, packetsQuery, values,
                    connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (packet.packet.isPacketOfType(pcpp::DHCP)) {
      buildDhcpv4Query(packet, dhcpv4Layer, firstField, dhcpv4_id, packetsQuery,
                       values, connection_local, prep_stmt, stmt, res);
      isProtocolSupported = true;
    }
    if (ethernet_id == -1 && ipv4_id == -1 && ipv6_id == -1 && arp_id == -1 &&
        icmp_id == -1 && tcp_id == -1 && udp_id == -1 && icmpv6_id == -1) {
      return;
    }
    ...
    LogController::getInstance()->addLog(
        Utils::getTime(), e.what() + std::string(" ") + __PRETTY_FUNCTION__,
        LogType::WARNING);
  }
}

void DataBaseController::buildIpv4Query(
    CapturedPackets &packet, std::shared_ptr<pcpp::IPv4Layer> &layer,
    bool &firstField, int &ipv4_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection_local,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::IPv4Layer>(
      *packet.packet.getLayerOfType<pcpp::IPv4Layer>());
  std::string query =
      R"(INSERT INTO ipv4(ipv4_version, ipv4_ihl, ipv4_tos, ipv4_total_length,
...
void DataBaseController::buildIpv6Query(
    CapturedPackets &packet, std::shared_ptr<pcpp::IPv6Layer> &layer,
    bool &firstField, int &ipv6_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::IPv6Layer>(
      *packet.packet.getLayerOfType<pcpp::IPv6Layer>());
  std::string query =
      R"(INSERT INTO ipv6(ipv6_version, ipv6_traffic_class, ipv6_flow_label, ipv6_payload_length,
  ...
  int combined =
      (static_cast<int>(layer->getIPv6Header()->flowLabel[0]) << 16) |
...
void DataBaseController::buildArpQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::ArpLayer> &layer,
    bool &firstField, int &arp_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::ArpLayer>(
      *packet.packet.getLayerOfType<pcpp::ArpLayer>());
  std::string query =
      R"(INSERT INTO arp(arp_hw_type, arp_proto_type, arp_hw_size, arp_proto_size,
...
void DataBaseController::buildIcmpQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::IcmpLayer> &layer,
    bool &firstField, int &icmp_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::IcmpLayer>(
      *packet.packet.getLayerOfType<pcpp::IcmpLayer>());
  std::string query =
      R"(INSERT INTO icmp(icmp_type, icmp_code, icmp_checksum) VALUES (?,?,?))";
  prep_stmt = connection->prepareStatement(query);
...
void DataBaseController::buildTcpQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::TcpLayer> &layer,
    bool &firstField, int &tcp_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::TcpLayer>(
      *packet.packet.getLayerOfType<pcpp::TcpLayer>());
  std::string query =
      R"(INSERT INTO tcp(tcp_src_port, tcp_dst_port, tcp_sequence_number
...
void DataBaseController::buildUdpQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::UdpLayer> &layer,
    bool &firstField, int &udp_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::UdpLayer>(
      *packet.packet.getLayerOfType<pcpp::UdpLayer>());
  std::string query =
      R"(INSERT INTO udp(udp_src_port, udp_dst_port, udp_length, udp_checksum) VALUES (?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);
...
void DataBaseController::buildEthernetQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::EthLayer> &layer,
    bool &firstField, int &ethernet_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection_local,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::EthLayer>(
      *packet.packet.getLayerOfType<pcpp::EthLayer>());
  std::string query = "INSERT INTO ethernet(ethernet_src_mac, "
                      "ethernet_dst_mac, ethernet_eht_type) VALUES (?,?,?)";
  ...
  prep_stmt->setString(
      3, std::to_string(layer->getEthHeader()->etherType).c_str());
  prep_stmt->executeUpdate();
  query = "";
  stmt = connection_local->createStatement();
  res = stmt->executeQuery("SELECT LAST_INSERT_ID()");
...
// void DataBaseController::buildSshQuery(CapturedPackets &packet,
// std::shared_ptr<pcpp::SSHLayer> &layer, bool firstField, int &ssh_id,
// std::string &packetsQuery, std::string &values) {
//     layer =
//     std::make_shared<pcpp::SSHLayer>(*packet.packet.getLayerOfType<pcpp::SSHLayer>());
//     std::string query = R"(INSERT INTO ssh(ssh_payload) VALUES (?,?))";
//     this->prep_stmt = this->connection->prepareStatement(query);
//
//     this->prep_stmt->setString(1, layer->toString()); // assuming SSH payload
//     is in string format
//
...
// void DataBaseController::buildSmtpQuery(CapturedPackets &packet,
// std::shared_ptr<pcpp::SmtpLayer> &layer, bool firstField, int &smtp_id,
// std::string &packetsQuery, std::string &values) {
//     layer =
//     std::make_shared<pcpp::SmtpLayer>(*packet.packet.getLayerOfType<pcpp::SmtpLayer>());
//     std::string query = R"(INSERT INTO smtp(smtp_payload) VALUES (?))";
//     this->prep_stmt = this->connection->prepareStatement(query);
//
//     this->prep_stmt->setString(1, layer->toString()); // assuming SMTP
...
//     values += "?";
// }
void DataBaseController::buildTelnetQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::TelnetLayer> &layer,
    bool &firstField, int &telnet_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::TelnetLayer>(
      *packet.packet.getLayerOfType<pcpp::TelnetLayer>());
  std::string query = R"(INSERT INTO telnet(telnet_payload) VALUES (?))";
  prep_stmt = connection->prepareStatement(query);

  prep_stmt->setString(
      1, layer->toString()); // assuming Telnet payload is in string format
...
void DataBaseController::buildDhcpv4Query(
    CapturedPackets &packet, std::shared_ptr<pcpp::DhcpLayer> &layer,
    bool &firstField, int &dhcpv4_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::DhcpLayer>(
      *packet.packet.getLayerOfType<pcpp::DhcpLayer>());
  std::string query =
      R"(INSERT INTO dhcpv4 (op_code, hardware_type, client_ip, your_ip, server_ip, gateway_ip, client_mac, options) VALUES (?,?,?,?,?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);
...
void DataBaseController::buildHttpResponseQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::HttpResponseLayer> &layer,
    bool &firstField, int &httpResponse_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::HttpResponseLayer>(
      *packet.packet.getLayerOfType<pcpp::HttpResponseLayer>());
  std::string query =
      R"(INSERT INTO http_responses(http_status_code, http_reason_phrase, http_version, http_headers, http_body) VALUES (?,?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);

  // Setting status code and reason phrase
  prep_stmt->setInt(1,
                    static_cast<int>(layer->getFirstLine()->getStatusCode()));
  ...
  for (auto field = layer->getFirstField(); field != nullptr;
       field = layer->getNextField(field)) {
  ...
  if (layer->getLayerPayloadSize() > 0) {
    std::string body(reinterpret_cast<const char *>(layer->getLayerPayload()),
...
void DataBaseController::buildHttpRequestQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::HttpRequestLayer> &layer,
    bool &firstField, int &httpRequest_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::HttpRequestLayer>(
      *packet.packet.getLayerOfType<pcpp::HttpRequestLayer>());
  std::string query =
      R"(INSERT INTO http_requests(http_url, http_version, http_method, http_headers, http_body) VALUES (?,?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);
...
void DataBaseController::buildDnsQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::DnsLayer> &layer,
    bool &firstField, int &dns_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::DnsLayer>(
      *packet.packet.getLayerOfType<pcpp::DnsLayer>());
  std::string query =
      R"(INSERT INTO dns(dns_transaction_id, dns_flags, dns_questions, dns_answer_rrs,
...
void DataBaseController::buildFtpRequestQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::FtpRequestLayer> &layer,
    bool &firstField, int &ftpRequest_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  layer = std::make_shared<pcpp::FtpRequestLayer>(
      *packet.packet.getLayerOfType<pcpp::FtpRequestLayer>());
  std::string query =
      R"(INSERT INTO ftp_requests(ftp_command, ftp_arguments, ftp_headers, ftp_payload) VALUES (?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);

  // Setting FTP command
  std::string ftpCommand =
      pcpp::FtpRequestLayer::getCommandAsString(layer->getCommand());
  ...
  std::stringstream headersStream;
  // Add code to retrieve headers if required. FTP typically does not have
  // headers like HTTP.
  std::string headers = headersStream.str();
  std::istringstream headersIss(headers);
  ...
  if (layer->getLayerPayloadSize() > 0) {
    std::string payload(
        reinterpret_cast<const char *>(layer->getLayerPayload()),
        layer->getLayerPayloadSize());
...
void DataBaseController::buildFtpResponseQuery(
    CapturedPackets &packet, std::shared_ptr<pcpp::FtpResponseLayer> &layer,
    bool &firstField, int &ftpResponse_id, std::string &packetsQuery,
    std::string &values, sql::Connection *&connection,
    sql::PreparedStatement *&prep_stmt, sql::Statement *&stmt,
    sql::ResultSet *&res) {
  // Initialize the ftpResponseLayer with data from the current packet
  layer = std::make_shared<pcpp::FtpResponseLayer>(
      *packet.packet.getLayerOfType<pcpp::FtpResponseLayer>());
  std::string query =
      R"(INSERT INTO ftp_responses(ftp_status_code, ftp_status_string, ftp_headers, ftp_payload) VALUES (?,?,?,?))";
  prep_stmt = connection->prepareStatement(query);
  ...
  std::string statusString =
      pcpp::FtpResponseLayer::getStatusCodeAsString(layer->getStatusCode());
